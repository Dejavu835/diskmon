...
final class HealthPredictor {
    ...
    func ingest(
        ...
        let criticalWarningDanger = Self.hardwareCriticalWarning(criticalWarningRaw)
        if criticalWarningDanger || mediaErrors > 0 || (percentageUsed.map { $0 >= 95 } ?? false) {
        ...
        if let level = immediate {
            ...
            if prev == .danger {
                ...
                let smartStillDangerous = Self.smartDanger(
                    criticalWarningRaw: criticalWarningRaw,
                    mediaErrors: mediaErrors,
    ...
    /// NVMe Critical Warning bits that mean the drive, not a temp dwell.
    /// bit0 spare, bit2 reliability, bit3 read-only, bit4 backup. Not bit1.
    static func hardwareCriticalWarning(_ raw: Int) -> Bool {
        (raw & 0x1D) != 0
    }

    static func smartDanger(
        criticalWarningRaw: Int,
        mediaErrors: Int,
        percentageUsed: Int?
    ) -> Bool {
        hardwareCriticalWarning(criticalWarningRaw)
            || mediaErrors > 0
            || (percentageUsed.map { $0 >= 95 } ?? false)
    }
