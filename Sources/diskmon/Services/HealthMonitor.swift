...
final class HealthMonitor {
    ...
    private func pollOnce() async {
        ...
        if smartctlMissing != ctlMissing { smartctlMissing = ctlMissing }
        ...
        if let uuid = targetUUID,
           disks.contains(where: { $0.volumeUUID == uuid }) {
            ...
            if let data = updatedSmartByUUID[uuid] {
                ...
                if let c = data.celsius, currentTemp != Double(c) { currentTemp = Double(c) }
            } else if current != SmartData() {
                current = SmartData()
            }
        }
        ...
        for (uuid, data) in updatedSmartByUUID {
            ...
            if prev != level {
                ...
                if let disk = disks.first(where: { $0.volumeUUID == uuid }) {
        ...
        let nextLevel = worstLevel
        if healthLevel != nextLevel { healthLevel = nextLevel }

        guard let context = modelContext else { return }
        let now = Date()
        for (uuid, data) in updatedSmartByUUID {
            // TemperatureSample has no reader. History is SmartSnapshot only.
            ...
            let snap = SmartSnapshot(
                ...
                percentageUsed: data.percentageUsed ?? -1,
                mediaErrors: data.mediaErrors ?? 0,
    ...
    private func refreshWatchedFromDiscovered() {
        ...
        if next != watchedDisks { self.watchedDisks = next }
    }
    ...
    private func stampIdentity(
        ...
        let nextWatched = watchedDisks.map(stamp)
        if nextWatched != watchedDisks { watchedDisks = nextWatched }
        let nextDiscovered = discoveredDisks.map(stamp)
        if nextDiscovered != discoveredDisks { discoveredDisks = nextDiscovered }
    }
    ...
    private func sampleIOOnce() async {
        ...
        for disk in disks {
            ...
            if let prev = last[disk.volumeUUID] {
                ...
                if dt > 0.2 {
                    ...
                    let floor = 8_192.0
                    let idle = readBps < floor && writeBps < floor
                    let wasIdle = (old?.readBps ?? 0) < floor && (old?.writeBps ?? 0) < floor
                    // 连续空闲不追加点、不改字典。活动开始或结束仍记一笔。
                    if !(idle && wasIdle) {
                        if old == nil
                            || abs((old?.readBps ?? 0) - readBps) > floor
                            || abs((old?.writeBps ?? 0) - writeBps) > floor {
        ...
        ioLastBytes = last
        if current != ioCurrentByUUID { ioCurrentByUUID = current }
        if history != ioHistoryByUUID { ioHistoryByUUID = history }
    ...
    private func enforceCacheLimit(context: ModelContext, now: Date) {
        ...
        var tempDescriptor = FetchDescriptor<TemperatureSample>(
            predicate: #Predicate<TemperatureSample> { s in
                s.timestamp < cutoff
            },
            sortBy: [SortDescriptor(\.timestamp, order: .forward)]
        )
        tempDescriptor.fetchLimit = 2000
        let temps = (try? context.fetch(tempDescriptor)) ?? []
        temps.forEach { context.delete($0) }
        guard !rows.isEmpty || !temps.isEmpty else { return }
        try? context.save()
        NSLog("DiskMon: cache limit prune deleted \(rows.count) snapshots + \(temps.count) temps (store ~\(size) bytes)")
    }
    ...
    private func persistHourBuckets() {
        ioHourBuckets = IOMean.cappedStore(ioHourBuckets, now: Date())
    ...
    private static func loadHourBuckets() -> [String: [IOBucket]] {
        ...
        return IOMean.cappedStore(dict, now: Date())
    }
    ...
    /// FS: only a volume that reported problems (`.warning`) is danger.
    /// `.failed` is the tool (bridge / exit 64+), not a dead filesystem.
    ...
    func evaluate(smart: SmartData, uuid: String? = nil) -> HealthLevel {
        ...
        if let integrity = disk?.integrity {
            ...
            case .warning: fsFailed = true
            case .failed, .verifying, .verified, .unknown: break
            }
        }
