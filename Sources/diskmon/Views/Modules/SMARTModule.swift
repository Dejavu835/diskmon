...
struct SMARTModule: View {
    ...
    private var fields: [SMARTField] {
        ...
        // bit1 (temperature) is not a hardware-death alert. bits 0/2/3/4 are.
        let hardwareAlert = ((critWarnVal ?? 0) & 0x1D) != 0
        let critWarnStatus: SMARTField.Status = hardwareAlert ? .danger : .normal
        let critWarnValue: String = {
            guard let v = critWarnVal else { return "—" }
            return hardwareAlert ? "ALERT:\(v)" : "\(v)"
        }()
