...
struct DiskManageContent: View {
    ...
    @State private var eraseWholeDisk: Bool = false
    ...
    var body: some View {
        ...
        .task {
            ...
            if volumeName.isEmpty, let d = target {
                ...
                personality = Self.personality(matching: d) ?? personality
            }
        }
    ...
    private var ntfsCard: some View {
        ...
        if let disk = target, NTFSAccess.isBitLocker(disk.filesystemName) {
            Text(L10n.t(
                "manage.ntfs.bitlocker",
                zh: "这台 Mac 解不开 BitLocker。去 Windows 解锁后再挂载。",
                en: "This Mac cannot unlock BitLocker. Unlock it on Windows, then mount.",
                language: lang
            ))
            .font(.system(size: 12))
            .foregroundStyle(Color.themeFgDark)
        ...
        NTFSStatusRow(
            ...
            onRemount: {
                ...
                Task { log = await formatter.enableNTFSWrite(disk: disk) }
            },
            onMount: {
                guard let disk = target else { return }
    ...
    private func formatCard(_ disk: DiskInfo) -> some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Text(eraseWholeDisk
                 ? L10n.t("manage.format.warn.whole", zh: "整盘抹掉，EFI 和恢复分区也会消失。只处理外接盘。", en: "Erases the whole disk, including EFI and recovery. External disks only.", language: lang)
                 : L10n.t("manage.format.warn.part", zh: "只抹当前数据分区，不动 EFI 和恢复分区。只处理外接盘。", en: "Erases the current data partition only. EFI and recovery stay. External disks only.", language: lang))
                .font(.system(size: 11))
                .foregroundStyle(.secondary)
                .fixedSize(horizontal: false, vertical: true)
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                Toggle(isOn: $eraseWholeDisk) {
                    Text(L10n.t("manage.format.whole", zh: "整盘抹掉", en: "Erase whole disk", language: lang))
                        .font(.system(size: 12))
                        .foregroundStyle(Color.themeFgDark)
                }
                .toggleStyle(.switch)
...
private extension DiskManageContent {
    static func personality(matching disk: DiskInfo) -> FormatPersonality? {
        switch disk.filesystemLabel {
        case "APFS": return .apfs
        case "ExFAT": return .exfat
        case "FAT32": return .fat32
        case "NTFS": return .ntfsKit
        default: return nil
        }
    }
}
