...
private struct HeroWidget: View {
    ...
    private var lifePercent: Int {
        guard let s = smart else { return 0 }
        return max(0, 100 - (s.percentageUsed ?? 0))
    }
        ...
        return VStack(spacing: 6) {
            HStack(spacing: 6) {
                ...
                heroStat(
                    ...
                    value: s.map { "\(max(0, 100 - ($0.percentageUsed ?? 0)))" } ?? "—",
                    unit: "%",
                    color: s.map { sd in
                        (sd.percentageUsed ?? 0) >= 90 ? Color.dsCritical
                        : (sd.percentageUsed ?? 0) >= 70 ? Color.dsWarning
                        : Color.themeFgDark
                    } ?? .secondary,
                    systemImage: "leaf"
                )
    ...
    private var rows: [Row] {
        ...
        return [
            ...
            Row(id: "pct", name: String(localized: "smart.percentageUsed", defaultValue: "Percentage Used"),
                ...
                status: (s.percentageUsed ?? 0) >= 90 ? .critical
                       : (s.percentageUsed ?? 0) >= 70 ? .warning : .normal),
...
struct HealthRingView: View {
    ...
    private var progressValue: CGFloat {
        guard let smart else { return 0 }
        let life = max(0, 100 - (smart.percentageUsed ?? 0))
        return CGFloat(life) / 100.0
    }
