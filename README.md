...
Mac 上看一块外接盘，常常要在磁盘工具、访达和各种检测软件之间来回切。diskmon 挂在菜单栏里，把这些收在一处：温度、SMART、健康度，以及眼下的读写。改名、推出、格式化这些日常操作也在里面。NTFS 盘需要写文件时，可以用自带的扩展挂成可写；不用的时候它不在后台跑。

传感器读不到就留空，不填一个假的 0°C。

4.0.1。macOS 14 及以上，只做 Apple Silicon。
